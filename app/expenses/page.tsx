    ...
    <div className="h-full flex flex-col max-w-5xl mx-auto w-full overflow-hidden space-y-4">
      ...
      <div className={`p-5 rounded-2xl border-2 shadow-md shrink-0 transition-colors ${
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {/* Desktop "+ Νέο Έξοδο" Button */}
          <button
            onClick={openCreate}
            className="hidden sm:flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black bg-amber-500 hover:bg-amber-400 text-white shadow-lg shadow-amber-500/30 transition-all hover:scale-105 active:scale-95 ring-2 ring-amber-400/40"
          >
            <Plus className="w-4 h-4 stroke-[3]" />
            <span>+ Νέο Έξοδο</span>
          </button>

          {/* Mobile Circular "+" Button at the end of the first line */}
          <button
            onClick={openCreate}
            className="sm:hidden w-8 h-8 rounded-full bg-amber-500 hover:bg-amber-400 text-white shadow-md shadow-amber-500/30 flex items-center justify-center transition-all active:scale-95 ring-2 ring-amber-400/40 cursor-pointer shrink-0"
            title="Νέο Έξοδο"
          >
            <Plus className="w-4 h-4 stroke-[3]" />
          </button>
      ...
      {byCategory.length > 0 && (
        <div className={`hidden sm:block p-4 rounded-2xl border ${isDark ? 'bg-slate-900/60 border-slate-800' : 'bg-white border-slate-200'}`}>
        ...
        <div className="space-y-2.5">
          {filtered.map(exp => (
            <div
              ...
              className={`p-3.5 sm:p-4 rounded-2xl border transition-all group ${
                isDark
                  ? 'bg-slate-900/80 border-slate-800 hover:border-amber-500/30 hover:bg-slate-800/80'
                  : 'bg-white border-slate-200 hover:border-amber-300 hover:shadow-md'
              }`}
            >
              {/* Top Row: Category & Date (Left) ── Amount & Actions (Right) */}
              ...
              {/* Bottom / Last Line: Comments Field */}
              <div className={`mt-2 pt-2 border-t text-xs font-medium flex items-start gap-1.5 ${
                isDark ? 'border-slate-800/60 text-slate-300' : 'border-slate-100 text-slate-700'
              }`}>
                <span className="text-slate-500 font-semibold shrink-0">Σχόλια:</span>
                <span className="break-words">{exp.comments || '-'}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <button
        onClick={openCreate}
        className="hidden sm:flex fixed bottom-6 right-6 z-40 p-4 rounded-full bg-amber-500 hover:bg-amber-400 text-white shadow-2xl shadow-amber-500/40 items-center gap-2 transition-all hover:scale-110 active:scale-95 border-2 border-amber-300"
        title="Νέο Έξοδο"
      >
        <Plus className="w-6 h-6 stroke-[3]" />
        <span className="font-black text-xs pr-1">Νέο Έξοδο</span>
      </button>
